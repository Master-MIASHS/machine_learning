...
>
	<TheorySection>
		...
		<p>
			...
			<KatexInline formula={String.raw`(X^{\mathrm{T}}X)^{-1}`} /> : il faut donc
			d’abord vérifier que cette inversion est licite. Géométriquement, la
			régression est la <strong>projection orthogonale</strong> de
		...
		<DefinitionBlock number="4.1" title="Matrice chapeau (hat matrix) et plein rang">
			<p>
				La matrice
				<KatexInline formula={String.raw`H = X(X^{\mathrm{T}}X)^{-1}X^{\mathrm{T}}`} />,
				appelée <strong>matrice chapeau</strong> (hat matrix), est la projection
				orthogonale sur
				...
				<KatexInline formula={String.raw`X^{\mathrm{T}}X`} /> n’est pas inversible
				et l’équation
			...
			<p>
				...
				<KatexInline formula="X" /> est très souvent de plein rang — une
				colinéarité <em>parfaite</em> est rare. En pratique, on vérifie le rang avec
				<code>qr(x)$rank</code> en R, puis on étudie la
				<strong>matrice des corrélations 2 à 2</strong> des prédicteurs pour
				repérer les colinéarités imparfaites.
			</p>
		</DefinitionBlock>
		...
		<p>
			Des prédicteurs corrélés entre eux partagent une information commune : le
			modèle « ne sait pas » à qui l’attribuer.
		</p>
		...
		<TheoremBlock number="4.2" title="Effets de la colinéarité entre prédicteurs">
			...
			<ul>
				<li>
					l’estimateur de
					<KatexInline formula={String.raw`\beta_j`} /> est proportionnel à la
					corrélation <strong>partielle</strong> entre
					...
					<KatexInline formula="Y" /> une fois retiré l’effet des autres
					caractéristiques ;
				</li>
				<li>
					et les estimateurs de deux variables corrélées ont une covariance
					négative :
					<KatexBlock formula={covCor} />
					les variables fortement corrélées se <strong>partagent
					l’importance</strong> dans le modèle.
				</li>
			</ul>
			<p>
				Conséquences : à l’extrême, avec une variable dupliquée
				<KatexInline formula={String.raw`Y = \beta_0 + \beta_1 X + \beta_2 X + \varepsilon = \beta_0 + (\beta_1 + \beta_2) X + \varepsilon`} />,
				les coefficients <KatexInline formula={String.raw`\beta_1, \beta_2`} /> ne
				sont <strong>pas identifiables</strong> (infinité de solutions) ; dans le
				cas général, les coefficients sont mal estimés, peuvent avoir des
				<strong>signes opposés</strong>, et certains passent sous le seuil de
				significativité. Numériquement,
				<KatexInline formula={String.raw`X^{\mathrm{T}}X`} /> est mal conditionnée
				: difficulté de convergence et manque de précision.
			</p>
		</TheoremBlock>

		<p>
			Les solutions : calculer le VIF de chaque variable, recourir à l’ACP pour
			identifier les liens, écart ou regroupement des prédicteurs posant problème
			(<em>un à un</em>), régression sur composantes principales, sélection de
			variables, ou régularisation — voir la
			<a href="/part5/lesson4">partie V</a>. Mais avant d’agir, on quantifie le
			problème avec deux indicateurs.
		</p>

		<DefinitionBlock number="4.3" title="Indice de conditionnement κ">
			<p>
				Soit <KatexInline formula="R" /> la matrice des corrélations empiriques
				des
				<KatexInline formula={String.raw`p`} /> variables explicatives, symétrique
				positive de rang
				...
				ses valeurs propres. S’il existe une relation linéaire parfaite entre un
				ensemble de variables, une des valeurs propres vaut
				<KatexInline formula="0" />. On définit l’indice de conditionnement
			</p>
			<KatexBlock formula={kappa} />
			<p>
				<strong>Règle courante :</strong>
				<KatexInline formula={String.raw`\kappa > 500`} /> signifie une
				colinéarité trop forte.
			</p>
		</DefinitionBlock>

		<DefinitionBlock number="4.4" title="Facteur d’inflation de la variance (VIF)">
			<p>
				Soit <KatexInline formula={String.raw`R_j^2`} /> le coefficient de
				détermination de la régression de
				...
				<KatexInline formula={String.raw`X_j`} /> n’est pas une fonction linéaire
				des autres, proche de
				<KatexInline formula="1" /> en cas de multi-colinéarité. Le VIF vaut
			</p>
			<KatexBlock formula={vif} />
			<p>
				<KatexInline formula={String.raw`\mathrm{VIF}_j`} /> est exactement le
				facteur par lequel la variance de
				<KatexInline formula={String.raw`\hat{\beta}_j`} /> est gonflée par rapport à
				ce qu’elle serait sans colinéarité :
			</p>
		...
		<InteractiveSection number="4.5" title="Colinéarité, conditionnement, VIF" onInteract={tracker.trackInteraction}>
		...
		<p>
			...
			<KatexInline formula="X" />. Pour les hypothèses sur le terme d’erreur —
			(H2) : erreurs centrées, de même variance, non corrélées ; (H3) : loi
			normale — on est « un peu malin » : on inspecte les
			<strong>résidus estimés</strong>
			<KatexInline formula={String.raw`\hat{\varepsilon}_i = y_i - \hat{y}_i`} />.
			Le diagnostic graphique est un outil puissant : il fournit des informations
			que les indicateurs statistiques appréhendent mal.
		</p>

		<DefinitionBlock number="4.6" title="Les résidus et leurs graphes">
			<p>
				Dans un modèle avec constante, la moyenne des résidus est
				<strong>nulle par construction</strong> : le vecteur des résidus est
				orthogonal aux variables explicatives
				<KatexInline formula={String.raw`\hat{\varepsilon}^{\mathrm{T}} X = 0`} />,
				et comme
			</p>
			<KatexBlock formula={resZero} />
			<p>
				cette moyenne nulle ne préjuge en rien de la pertinence de la
				régression. Pour un modèle valide, les graphes attendus — résidus
				toujours en ordonnée — sont :
			</p>
			<ul>
				<li>
					<strong>vs la réponse</strong>
					<KatexInline formula="Y" /> : les résidus répartis aléatoirement
					autour de 0, sans valeur différente selon
					<KatexInline formula="Y" /> ;
				</li>
				<li>
					<strong>vs chaque variable explicative</strong>
					<KatexInline formula={String.raw`X_i`} /> : aucune relation entre le
					terme d’erreur et les variables explicatives (indépendance par
					hypothèse, à confirmer visuellement) ;
				</li>
				<li>
					<strong>vs le temps</strong> (données temporelles) : détection de
					ruptures de structure et d’auto-corrélations.
				</li>
			</ul>
			<p>
				Les pathologies classiques ont leur signature :
				<strong>asymétrie</strong> (plusieurs populations mélangées, ou variable
				importante manquante), <strong>blocs</strong> (non-linéarité),
				...
				<KatexInline formula={String.raw`\texttt{lowess()}`} /> et un
				histogramme.
			</p>
		</DefinitionBlock>

		<InteractiveSection number="4.7" title="La clinique des résidus" onInteract={tracker.trackInteraction}>
		...
		<p>
			...
			<KatexInline formula={String.raw`(x_{j,i}, y_i)`} /> n’est pas fiable : les
			autres variables explicatives influencent aussi
			<KatexInline formula="Y" /> et brouillent l’impression. On veut le lien entre
			<KatexInline formula={String.raw`X_j`} /> et
			<KatexInline formula="Y" />
			<strong>conditionnellement</strong> aux autres variables — d’où les résidus
			partiels.
		</p>
		...
		<DefinitionBlock number="4.8" title="Résidus partiels">
			...
			<p>
				: il <strong>enlève l’effet estimé</strong> de toutes les variables
				explicatives autres que
				<KatexInline formula={String.raw`X_j`} /> sur
				...
				: s’il s’ajuste par une droite, le lien est linéaire ; sinon, il est
				non-linéaire — on cherche alors une transformation de
				...
				<KatexInline formula={String.raw`\texttt{ln()}`} />…) à remplacer ou à
				ajouter au modèle.
			</p>
			<p>
				Enfin, le <strong>Q-Q plot</strong> des résidus — quantiles observés
				contre quantiles théoriques de
				<KatexInline formula={String.raw`\mathcal{N}(0,1)`} /> — permet de se
				faire une idée de la <strong>normalité</strong> des résidus, c’est-à-dire
				de vérifier (H3) : des points alignés sur la diagonale, une déviation en
				queues signale une asymétrie.
			</p>
		</DefinitionBlock>

		<InteractiveSection number="4.9" title="Résidus partiels : voir le lien masqué" onInteract={tracker.trackInteraction}>
		...
		<DefinitionBlock number="4.10" title="Atypique, levier, influent">
			<ul>
				<li>
					un point <strong>atypique</strong> (ou aberrant) s’écarte résolument
					des autres — erreur de recueil, individu hors population, ou simple
					variabilité d’échantillonnage ;
				</li>
				<li>
					un point est un <strong>levier</strong> quand les variables
					explicatives
					<KatexInline formula={String.raw`x_i`} /> sont éloignées du
					barycentre : il pèse fort sur la prédiction, même avec un résidu
					modéré ;
				</li>
				<li>
					un point est <strong>influent</strong> quand son écart modifie
					sensiblement les coefficients — combinaison levier <em>et</em>
					résidu.
				</li>
			</ul>
			<p>
				<strong>Un point atypique n’est pas forcément influent</strong> : un
				gros résidu au centre du nuage ne déplace guère la droite, tandis qu’un
				résidu modéré en bout de nuage peut la faire pivoter. Le moyen sûr de
				le vérifier : recalculer la régression en écartant le point.
			</p>
		</DefinitionBlock>
		...
		<TheoremBlock number="4.11" title="Leviers et matrice chapeau">
			<p>
				...
				<KatexInline formula={String.raw`\mathrm{V}(\hat{Y}) = \sigma^2 H`} /> :
				la variance du
				<KatexInline formula={String.raw`i`} />-ème ajusté vaut
				<KatexInline formula={String.raw`h_{ii}\,\sigma^2`} />. La composante
				diagonale
			...
			<p>
				...
				; et
				<KatexInline formula={String.raw`\sum_i h_{ii} = \mathrm{tr}(H) = \mathrm{rang}(H) = p+1`} />).
				En pratique,
		...
		<DefinitionBlock number="4.12" title="Résidus standardisés et studentisés">
			<p>
				...
				<strong>pas homoscédastiques</strong>. Pour les rendre comparables, on
				les standardise :
			</p>
			<KatexBlock formula={residStd} />
			<p>
				Mais
				<KatexInline formula={String.raw`\hat{\varepsilon}_i`} /> n’est pas
				indépendant de
				<KatexInline formula={String.raw`\hat{\sigma}`} /> — on remplace donc
				<KatexInline formula={String.raw`\hat{\sigma}`} /> par
				<KatexInline formula={String.raw`\hat{\sigma}^{(-i)}`} />, l’estimation
				obtenue <strong>sans l’observation</strong>
				<KatexInline formula="i" /> :
			</p>
			<KatexBlock formula={residStud} />
			<p>
				(numérateur et dénominateur indépendants ; sous (H3), loi de Student à
				<KatexInline formula={String.raw`n-p-2`} /> degrés de liberté). En
				pratique,
		...
		<TheoremBlock number="4.13" title="Distance de Cook">
			<p>
				...
				<KatexInline formula={String.raw`D_i`} /> est la distance (normalisée)
				entre les ajustements
				<KatexInline formula={String.raw`\hat{Y}`} /> et
				<KatexInline formula={String.raw`\hat{Y}^{(-i)}`} /> obtenus en
				écartant l’observation
				<KatexInline formula="i" />. Elle se calcule
				<strong>sans refaire</strong>
				<KatexInline formula="n" /> régressions, comme fonction des leviers et
			...
			<p>
				...
				<strong>influen<em>te</em></strong> — puis l’expliquer en regardant, pour
				ces observations, leur résidu studentisé (atypique ?) et leur levier
				(éloigné ?).
			</p>
		</TheoremBlock>
		...
		<ExampleBlock number="4.14" title="Que faire des observations influentes ?">
			...
			<ul>
				<li>
					<strong>supprimer</strong> l’observation (attention : cela peut
					rendre influentes de <em>nouvelles</em> observations) ;
				</li>
				<li>
			...
			<p>
				La décision dépend du contexte et doit être <strong>argumentée</strong> —
				par exemple, un point levier peut tout à fait appartenir à la population
				(viabilité d’échantillonnage).
			</p>
		</ExampleBlock>

		<InteractiveSection number="4.15" title="Leviers, résidus studentisés et distance de Cook" onInteract={tracker.trackInteraction}>
		...
		<DefinitionBlock number="4.16" title="Checklist de validation du modèle linéaire">
			...
			<ul>
				<li>pas de multi-colinéarité dans la matrice <KatexInline formula="X" /> (plein rang, conditionnement, VIF) ;</li>
				<li>relations <strong>linéaires</strong> entre variables explicatives et variable à expliquer (résidus partiels) ;</li>
				<li>résidus <strong>« sans structure »</strong> : variance constante, indépendance des observations, distribution symétrique (Q-Q plot) ;</li>
				<li>pas d’observation aberrante ou trop influente (
		...
		<Callout type="note" title="Vers la suite">
			<p>
				...
				<strong>sélectionner</strong> le bon sous-ensemble de variables et à
				l’interpréter — critères de comparaison, algorithmes de sélection, dans
				la <a href="/part4/lesson5">leçon 5</a>.
			</p>
		</Callout>
	</TheorySection>
