<script lang="ts">
	...
	// ── Formules (course_sources/sophie/ModèleLinéaire_ANOVA_ANCOVA.pdf) ──
	...
	<TheorySection>
		...
		<ul>
			<li>
				...
				<strong>régresseurs</strong> : des variables quantitatives (on explique par exemple la teneur
				en ozone par la température ou la nébulosité) ;
			</li>
			<li>
				les modèles d'<strong>analyse de la variance</strong> (ANOVA) — les covariables sont des
				<strong>facteurs</strong> : des variables qualitatives (on explique la teneur en ozone par la
				direction du vent ou la saison).
			</li>
		</ul>

		<p>
			Tout dépend donc de la matrice <KatexInline formula="X" /> des covariables… mais
			<KatexInline formula="Y" /> ne change pas : on cherche dans les deux cas à expliquer une
			<strong>mesure numérique</strong>. On peut même mélanger les deux natures de covariables — ce
			sont les modèles d'<strong>analyse de covariance</strong>
			(ANCOVA), dans la dernière section de cette leçon.
		</p>

		<DefinitionBlock number="2.1" title="Régresseur et facteur">
			<p>
				Un <strong>régresseur</strong> prend des valeurs numériques : une infinité de valeurs
				possibles, souvent toutes différentes dans l'échantillon — donc une valeur de <KatexInline
					formula="Y"
				/> différente pour chaque valeur de
				<KatexInline formula="X" />. Un <strong>facteur</strong> prend des
				<strong>niveaux</strong> : un nombre fini et faible de modalités — les valeurs observées de <KatexInline
					formula="Y"
				/> se répètent dans chaque niveau.
			</p>
		</DefinitionBlock>
		...
		<p>
			...
			<KatexInline formula={String.raw`Y_i = \beta_0 + \beta_1 x_i + \varepsilon_i`} />, on cherche
			la part de variabilité de <KatexInline formula="Y" /> induite par les variations du régresseur :
			un paramètre par régresseur, où
			...
			<KatexInline formula={String.raw`\beta_1`} /> la pente (impact d'un accroissement de 1 de <KatexInline
				formula="x"
			/>).
		</p>
		...
		<p>
			...
			<KatexInline formula="k" /> dans le niveau <KatexInline formula="j" />, et le modèle d'ANOVA
			est
		</p>
		...
		<p>
			on cherche la part de variabilité de <KatexInline formula="Y" /> induite par les différents niveaux
			du facteur. Il y a <strong>un paramètre par niveau</strong> du facteur :
		</p>
		<ul>
			<li>
		...
		<p>
			...
			exige que <KatexInline formula={String.raw`X^\top X`} /> soit inversible, c'est-à-dire que
			...
			<KatexInline formula="p" />. Avec un facteur à <KatexInline formula="J" /> niveaux, on ne peut pas
			prendre la colonne de chaque niveau <em>et</em> une constante — les colones seraient
			linéairement dépendantes. On impose donc une <strong>contrainte</strong>, et il en existe
			trois usuelles :
		</p>

		<DefinitionBlock number="2.2" title="Contrainte : sans intercept">
			<p>
				<KatexInline formula={String.raw`\beta_0 = 0`} /> (pas de colonne de constantes). Pour tout niveau
				<KatexInline formula={String.raw`j \in \{1, \dots, J\}`} />,
		...
		<DefinitionBlock number="2.3" title="Contrainte : niveau de référence">
			<p>
				<KatexInline formula={String.raw`\beta_1 = 0`} /> — le niveau 1 sert de référence : <KatexInline
					formula={String.raw`\beta_0`}
				/> est la moyenne de la réponse dans le niveau 1, et pour
				<KatexInline formula={String.raw`j \in \{2, \dots, J\}`} />,
				<KatexInline formula={String.raw`\beta_j`} /> est l'<strong>écart</strong> de la réponse du
				niveau <KatexInline formula="j" /> au niveau 1. C'est le codage
		...
		<DefinitionBlock number="2.4" title="Contrainte : somme des paramètres">
			<p>
				...
				<KatexInline formula={String.raw`\beta_0`} /> est la <strong>moyenne des paramètres</strong>
				(moyenne générale), pour
				<KatexInline formula={String.raw`j \in \{1, \dots, J-1\}`} /> la moyenne du niveau <KatexInline
					formula="j"
				/> vaut
				<KatexInline formula={String.raw`\beta_0 + \beta_j`} />, et celle du niveau
				<KatexInline formula="J" /> vaut
				<KatexInline formula={String.raw`\beta_0 - \sum_{j=1}^{J-1}\beta_j`} />. C'est le codage <KatexInline
					formula={String.raw`\texttt{contr.sum}`}
				/> de R.
			</p>
		</DefinitionBlock>
		...
		<p>
			...
			<KatexInline formula={String.raw`\hat{Y}`} /> (et les moyennes par niveau) est le même dans les
			trois cas, seules la matrice <KatexInline formula="X" /> et l'interprétation des <KatexInline
				formula={String.raw`\beta_j`}
			/> changent.
		</p>

		<InteractiveSection
			number="2.5"
			title="Trois codages d'un facteur"
		...
		<p>
			...
			<KatexInline formula={String.raw`J_2`} /> modalités) deux facteurs, avec plusieurs observations
			de <KatexInline formula="Y" /> à chaque <strong>croisement</strong> d'une modalité de <KatexInline
			...
			<KatexInline formula="j" /> de <KatexInline formula={String.raw`F_2`} />. Le modèle à
			<strong>effets principaux</strong> est
		</p>
		...
		<p>
			...
			<KatexInline formula="j" /> de <KatexInline formula={String.raw`F_2`} />. Contraintes usuelles
		...
		<p>
			...
			<KatexInline formula={String.raw`F_2`} />, et réciproquement. Mais il arrive que le niveau <KatexInline
				formula="i"
			/> de
			<KatexInline formula={String.raw`F_1`} /> ait un
			<strong>effet différent selon le niveau</strong>
			de <KatexInline formula={String.raw`F_2`} /> : par exemple, l'orientation du vent n'a pas le même
			effet avec ou sans pluie. On ajoute alors les termes d'<strong>interaction</strong>
		...
		<ExampleBlock number="2.6" title="Exemple : vent × pluie">
			<p>
				Pour une parcelle, la teneur en ozone dépend de la direction du vent (<KatexInline
					formula={String.raw`F_1`}
				/>) et de la présence de pluie (<KatexInline formula={String.raw`F_2`} />). Sans
				interaction, l'effet d'un vent de nord est identique qu'il pleuve ou non ; avec interaction
				<KatexInline formula={String.raw`\gamma_{ij}`} />, l'effet du vent de nord peut être nul par
				temps sec et fort par temps de pluie — l'effet de
		...
		<InteractiveSection
			number="2.7"
			title="Interaction entre deux facteurs"
			onInteract={tracker.trackInteraction}
		>
		...
		<p>
			Dans un modèle d'<strong>analyse de covariance</strong>, la partie explicative mélange un
			régresseur <KatexInline formula="X" /> et un facteur
		...
		<p>
			: l'effet du facteur n'a d'impact <strong>que sur l'intercept</strong> — les droites de
			régression de chaque niveau sont <strong>parallèles</strong>. Avec interaction <KatexInline
				formula={String.raw`Y \sim X * F`}
			/>,
		</p>
		...
		<p>
			: l'effet du facteur a un impact sur l'intercept <strong>et</strong> sur la pente — les droites
			de chaque niveau ne sont plus parallèles.
		</p>
		...
		<Callout type="insight" title="Dans R">
			<p>
				...
				<code>anova()</code> appliqué à un <code>lm</code> renvoie le tableau d'ANOVA avec les
				sommes de carrés par effet (tandis que
				<code>summary()</code> donne les tests t coefficient par coefficient) — à comparer à la section
				« Sommes de carrés et coefficient de détermination » de la leçon précédente.
			</p>
		</Callout>

		<InteractiveSection
			number="2.8"
			title="ANCOVA : droites parallèles ou pentes propres ?"
			onInteract={tracker.trackInteraction}
		>
			<DeferredDemo load={() => import('$lib/components/demos/LmAncova.svelte')} />
		</InteractiveSection>

		<Callout type="note" title="Vers la suite">
			<p>
				Avec des erreurs gaussiennes, on pourra tester si un facteur (ou une interaction) est
				significatif — le <strong>test F</strong> de la
				<a href="/part4/lesson3">leçon 3</a>. Et si les facteurs sont corrélés entre eux, le
				diagnostic de la <a href="/part4/lesson4">leçon 4</a> s'appliquera à la matrice de design complète.
			</p>
		</Callout>
	</TheorySection>
</PageTemplate>
